...
bool PromptCoincidence(TGriffinHit *h1, TGriffinHit *h2){  //Check if GRIFFIN hits are less then 120 ns apart.
    return std::fabs(h1->GetTime() - h2->GetTime()) < 120.;
}
// time random
...
void TwoPhotonGriffinSelector::CreateHistograms() {
...
void TwoPhotonGriffinSelector::FillHistograms() {
    //GRIFFIN non-addback
    ...
    for(auto g = 0; g < fGrif->GetAddbackMultiplicity(); ++g) {
        ...
        fH2.at("addbackE")->Fill(grif->GetDetector(), grif->GetEnergy()); 

        //GRIFFIN-GRIFFIN
        for(auto g2 = g+1; g2 < fGrif->GetAddbackMultiplicity(); ++g2) {
            auto grif2 = fGrif->GetAddbackHit(g2);
            if(PromptCoincidence(grif, grif2)) {
                fH2.at("addbackAddbackE")->Fill(grif->GetEnergy(), grif2->GetEnergy());
                fH2.at("addbackAddbackE")->Fill(grif2->GetEnergy(), grif->GetEnergy());
            } else if(TimeRandom(grif, grif2)) {
                fH2.at("addbackAddbackEBg")->Fill(grif->GetEnergy(), grif2->GetEnergy());
                fH2.at("addbackAddbackEBg")->Fill(grif2->GetEnergy(), grif->GetEnergy());
            }
        }
    ...
    if (fGrif->GetMultiplicity() == 2){
        for(auto g = 0; g < fGrif->GetMultiplicity(); ++g) {
            ...
            for(auto g2 = g+1; g2 < fGrif->GetMultiplicity(); ++g2) {
                auto grif2 = fGrif->GetGriffinHit(g2);
                if (PromptCoincidence(grif, grif2)){
                    if (LowEnergyThreshold(grif,grif2)) {
                        fH1.at("griffinESum")->Fill(grif->GetEnergy() + grif2->GetEnergy());
                    }
                    if (AggressiveEnergyThreshold(grif,grif2)) {
                        fH1.at("griffinESum_Aggr")->Fill(grif->GetEnergy() + grif2->GetEnergy());
                    }
                    if (EnergyDiffThreshold(grif,grif2) && AngleRestriction(grif, grif2)) {
                        fH1.at("griffinESum_PRL")->Fill(grif->GetEnergy() + grif2->GetEnergy());
                    }
                }
            }
        }
    ...
    //GRIFFIN Crystal Sum (Suppressed)
    for(auto g = 0; g < fGrif->GetSuppressedMultiplicity(fBgo); ++g){
        auto grif = fGrif->GetSuppressedHit(g);
        fH2.at("griffinESupp")->Fill(grif->GetArrayNumber(), grif->GetEnergy());

    }
    if (fGrif->GetSuppressedMultiplicity(fBgo) == 2){
        for(auto g = 0; g < fGrif->GetMultiplicity(); ++g) {
            auto grif = fGrif->GetGriffinHit(g);
            for(auto g2 = g+1; g2 < fGrif->GetMultiplicity(); ++g2) {
                auto grif2 = fGrif->GetGriffinHit(g2);
                if (PromptCoincidence(grif,grif2) && LowEnergyThreshold(grif,grif2)){
                    fH1.at("griffinESum_Supp")->Fill(grif->GetEnergy()+grif2->GetEnergy());
