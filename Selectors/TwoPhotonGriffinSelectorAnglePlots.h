...
#ifndef TwoPhotonGriffinSelectorAnglePlots_h
#define TwoPhotonGriffinSelectorAnglePlots_h
...
class TwoPhotonGriffinSelectorAnglePlots : public TGRSISelector { //Must be same name as .C and .h
    ...
    public :
        ...
        TwoPhotonGriffinSelectorAnglePlots(TTree * /*tree*/ =0) : TGRSISelector(), fGrif(NULL), fBgo(NULL), fZds(NULL), fScep(NULL), fLaBr(NULL), fLaBrBgo(NULL), fTAC(NULL), fDesc(NULL) {
            SetOutputPrefix("TwoPhotonGriffinAnglePlots"); //Changes prefix of output file
        }
        //These functions are expected to exist
        virtual ~TwoPhotonGriffinSelectorAnglePlots() { }
        virtual Int_t   Version() const { return 2; }
        void CreateHistograms();
        void FillHistograms();
        void InitializeBranches(TTree *tree);

        ClassDef(TwoPhotonGriffinSelectorAnglePlots,2); //Makes ROOT happier
};

#endif

#ifdef TwoPhotonGriffinSelectorAnglePlots_cxx
void TwoPhotonGriffinSelectorAnglePlots::InitializeBranches(TTree* tree)
{
    if(!tree) return;
...
#endif // #ifdef TwoPhotonGriffinSelectorAnglePlots_cxx
