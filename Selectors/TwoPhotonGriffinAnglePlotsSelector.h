...
#ifndef TwoPhotonGriffinAnglePlotsSelector_h
#define TwoPhotonGriffinAnglePlotsSelector_h
...
class TwoPhotonGriffinAnglePlotsSelector : public TGRSISelector { //Must be same name as .C and .h
    ...
    public :
        ...
        TwoPhotonGriffinAnglePlotsSelector(TTree * /*tree*/ =0) : TGRSISelector(), fGrif(NULL), fBgo(NULL), fZds(NULL), fScep(NULL), fLaBr(NULL), fLaBrBgo(NULL), fTAC(NULL), fDesc(NULL) {
            SetOutputPrefix("TwoPhotonGriffinAnglePlots"); //Changes prefix of output file
        }
        //These functions are expected to exist
        virtual ~TwoPhotonGriffinAnglePlotsSelector() { }
        virtual Int_t   Version() const { return 2; }
        void CreateHistograms();
        void FillHistograms();
        void InitializeBranches(TTree *tree);

        ClassDef(TwoPhotonGriffinAnglePlotsSelector,2); //Makes ROOT happier
};

#endif

#ifdef TwoPhotonGriffinAnglePlotsSelector_cxx
void TwoPhotonGriffinAnglePlotsSelector::InitializeBranches(TTree* tree)
{
    if(!tree) return;
