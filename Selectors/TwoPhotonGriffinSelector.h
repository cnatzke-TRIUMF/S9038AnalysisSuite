...
#ifndef TwoPhotonGriffinSelector_h
#define TwoPhotonGriffinSelector_h
...
class TwoPhotonGriffinSelector : public TGRSISelector { //Must be same name as .C and .h
    ...
    public :
        ...
        TwoPhotonGriffinSelector(TTree * /*tree*/ =0) : TGRSISelector(), fGrif(NULL), fBgo(NULL), fZds(NULL), fScep(NULL), fLaBr(NULL), fLaBrBgo(NULL), fTAC(NULL), fDesc(NULL) {
            SetOutputPrefix("TwoPhotonGriffin"); //Changes prefix of output file
        }
        //These functions are expected to exist
        virtual ~TwoPhotonGriffinSelector() { }
        virtual Int_t   Version() const { return 2; }
        void CreateHistograms();
        void FillHistograms();
        void InitializeBranches(TTree *tree);

        ClassDef(TwoPhotonGriffinSelector,2); //Makes ROOT happier
};

#endif

#ifdef TwoPhotonGriffinSelector_cxx
void TwoPhotonGriffinSelector::InitializeBranches(TTree* tree)
{
    if(!tree) return;
...
#endif // #ifdef TwoPhotonGriffinSelector_cxx
